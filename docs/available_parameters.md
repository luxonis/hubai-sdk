...
| `quantization_data`       | `Literal["DRIVING", "FOOD", "GENERAL", "INDOORS", "RANDOM", "WAREHOUSE", "CLIP", "UNKNOWN"] \| Dataset ID \| local .zip path \| None` | The data used to quantize this `ModelInstance`. Can be a predefined domain, a dataset ID (`aid_...`), or `CUSTOM` when using an uploaded quantization zip.                  |
