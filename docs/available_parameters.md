...
| `quantization_data`       | `Literal["DRIVING", "FOOD", "GENERAL", "INDOORS", "RANDOM", "WAREHOUSE", "CLIP", "CUSTOM", "UNKNOWN"] \| Dataset ID \| GCS path \| None` | The data used to quantize this `ModelInstance`. Can be a predefined domain, a dataset ID (`aid_...`), a `gs://...` path, or `CUSTOM` when using an uploaded quantization zip. |
