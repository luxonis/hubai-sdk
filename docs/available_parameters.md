...
| `quantization_data`       | `Literal["DRIVING", "FOOD", "GENERAL", "INDOORS", "RANDOM", "WAREHOUSE", "CLIP", "UNKNOWN"] \| Dataset ID \| local .zip path \| None` | The data used to quantize this `ModelInstance`. Can be a predefined domain, a dataset ID, or a local `.zip` path.                                                           |
