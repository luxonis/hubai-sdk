...
| `quantization_data`       | `Literal["DRIVING", "FOOD", "GENERAL", "INDOORS", "RANDOM", "WAREHOUSE", "CLIP", "UNKNOWN"] \| Dataset ID \| GCS path \| local .zip path \| None` | The data used to quantize this `ModelInstance`. Can be a predefined domain, a dataset ID (`aid_...`), a custom GCS path, or a local `.zip` path that the SDK uploads before export. |
